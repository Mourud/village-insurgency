...
export default class Player {
  ...
  movesLeft: any;
  goldAmount: any;
  constructor(color: string, population: number, food: number, gold: number) {
    this.color = color;
    this.registry = [];
  ...
  checkClash(position: { x: number; y: number }) {
  ...
  collidesPosition(pos1: {x: number, y:number}, pos2: {x: number, y:number}) {
  ...
  collidesXY(x1:number, x2:number) {
